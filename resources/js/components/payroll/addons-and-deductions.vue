            ...
            <div class="table-responsive">
                <table class="table table-hover table-sm table-xs table-bordered" id="response">
                    <thead>
                        ...
                        <th class='text-center'>Addon Description</th>
                    ...
                    <tbody>
                        <tr v-for="(employee, index) in employees" :key="employee.id">
                            ...
                            <td>
                                <input class="table-input" :class="tableInputTextColor" v-model="employee.AddonName" @keyup.enter="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" @blur="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" type="text"/>
                            </td>
                            <td>
                                <input class="table-input text-right" :class="tableInputTextColor" v-model="employee.AddonAmount" @keyup.enter="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" @blur="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" type="number" step="any"/>
                            </td>
                            <td>
                                <input class="table-input" :class="tableInputTextColor" v-model="employee.DeductionName" @keyup.enter="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" @blur="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" type="text"/>
                            </td>
                            <td>
                                <input class="table-input text-right" :class="tableInputTextColor" v-model="employee.DeductionAmount" @keyup.enter="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" @blur="inputEnter(employee.AddonName, employee.AddonAmount, employee.DeductionName, employee.DeductionAmount, employee.id)" type="number" step="any"/>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    methods : {
        ...
        getOtherDeductionData() {
            if (this.isNull(this.schedule)) {
                ...
                axios.get(`${ axios.defaults.baseURL }/other_payroll_deductions/get-addons-and-deductions`, {
                    params: {
                    ...
                    for (let i=0; i<size; i++) {
                        this.employees.push({
                            ...
                            DeductionName : response.data[i]['DeductionName'],
                            DeductionAmount : response.data[i]['DeductionAmount'],
                            AddonName : response.data[i]['AddonName'],
                            AddonAmount : response.data[i]['AddonAmount'],
        ...
        inputEnter(addonName, addonAmount, deductionName, deductionAmount, employeeId) {
            if (this.isNull(this.schedule)) {
                this.toast.fire({
                    icon : 'warning',
                    text : 'Please select payroll schedule!'
                ...
                axios.get(`${ axios.defaults.baseURL }/other_payroll_deductions/update-addons-and-deductions`, {
                    params: {
