...
<body class="hold-transition sidebar-mini layout-fixed {{ $userCache->ColorProfile }}">
    <div class="wrapper">
        <!-- Main Header -->
        <nav
            class="main-header navbar navbar-expand {{ $userCache->ColorProfile != null ? 'navbar-dark' : 'navbar-light' }}">
            <!-- Left navbar links -->
            <ul class="navbar-nav">
                <li class="nav-item">
            ...
            <ul class="navbar-nav ml-auto">
                ...
                @if (!in_array(Route::currentRouteName(), ['tripTickets.log-vehicle-trips', 'tripTickets.log-vehicle-arrivals']))
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="file-menu" role="button"
                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">File </a>
                        <div class="dropdown-menu" aria-labelledby="file-menu">
                            <a href="{{ route('leaveApplications.file-leave') }}" class="dropdown-item"
                                title="File for leave">Leave for Myself</a>
                            @canany('create leave for others')
                                <a href="{{ route('leaveApplications.file-for-coworker') }}" class="dropdown-item"
                                    title="File for leave">Leave for my Co-Worker</a>
                            @endcanany
                            <a href="{{ route('tripTickets.create') }}" class="dropdown-item"
                                title="Make a trip ticket">Trip Ticket</a>
                            <a href="{{ route('travelOrders.create') }}" class="dropdown-item"
                                title="Make a travel order">Travel Order</a>
                            <a href="{{ route('offsetApplications.create') }}" class="dropdown-item"
                                title="Claim an offset">Offset</a>
                            <a href="{{ route('overtimes.create') }}" class="dropdown-item"
                                title="File for an overtime">Overtime Authorization</a>
                            <a href="{{ route('attendanceConfirmations.create') }}" class="dropdown-item"
                                title="File for an attendance confirmation">Attendance Confirmation</a>
                            <a href="{{ route('leaveConversions.create') }}" class="dropdown-item"
                                title="File for a leave to cash conversion">Leave Conversion</a>
                        </div>
                    </li>
                @endif
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                        <span class="dropdown-item dropdown-header"><span id="notif-count-expand">0</span>
                            Notification(s)</span>
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link" href="{{ route('tutorials.index') }}" title="Help center">
                ...
                <li class="nav-item dropdown user-menu">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">
                        @if ($profilePic != null)
                            <img src="{{ $profilePic }}" class="user-image img-circle img-cover" alt="User Image">
                        @else
                            <img src="{{ URL::asset('imgs/prof-img.png') }}" class="user-image img-circle img-cover"
                                alt="User Image">
                        @endif
                    ...
                    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                        ...
                        <li class="user-header">
                            ...
                            @if ($profilePic != null)
                                <img src="{{ $profilePic }}" class="user-image img-circle img-cover"
                                    alt="User Image">
                            @else
                        ...
                        <table class="table table-borderless table-hover table-sm">
                            <tr>
                                <td>
                                    <a href="{{ route('users.my-account-index', [Auth::user()->employee_id]) }}"
                                        class="btn btn-link {{ $userCache->ColorProfile != null ? 'text-light' : 'text-dark' }}"><i
                                            class="fas fa-user-circle ico-tab"></i>My Account</a>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    ...
                                    <div class="custom-control custom-switch"
                                        style="margin-left: 10px; margin-top: 6px; margin-bottom: 6px;">
                                        <input type="checkbox" {{ $userCache->ColorProfile != null ? 'checked' : '' }}
                                            class="custom-control-input" id="color-switch">
                                        <label style="font-weight: normal;" class="custom-control-label"
                                            for="color-switch" id="color-switchLabel">Dark Mode</label>
                                    </div>
                            ...
                            <tr>
                                <td style="border-top: 1px solid #e6e6e6;">
                                    <a href="#"
                                        class="btn btn-link {{ $userCache->ColorProfile != null ? 'text-light' : 'text-dark' }}"
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, element) {
                        $('#BarangayCurrent').append("<option value='" + element + "' " + (element ==
                                prevValue ? "selected='selected'" : " ") + ">" + index +
                            "</option>");
                    });
                },
        ...
        function fetchBarangayFromTownPermanent(townId, prevValue) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, element) {
                        $('#BarangayPermanent').append("<option value='" + element + "' " + (element ==
                                prevValue ? "selected='selected'" : " ") + ">" + index +
                            "</option>");
                    });
                },
