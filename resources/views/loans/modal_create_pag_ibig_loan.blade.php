<div class="modal fade" id="modal-pag-big-loan" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">New Pag-Ibig Loan</h4>
...
@push('page_scripts')
    <script>
        $('#save-loan').on('click', function() {
            $.ajax({
                url : "{{ route('loans.save-pag-ibig-loans') }}",
                type : 'GET',
                data : {
                ...
                error : function(err) {
                    Toast.fire({
                        icon : 'error',
                        text : 'Error saving Pag-ibig loan!'
                    })
                }
            })
        })

        $('#modal-pag-big-loan').on('shown.bs.modal', function (e) {
        ...
        $('#modal-pag-big-loan').on('hidden.bs.modal', function (e) {
            $('#Amount').val('')
            $('#Terms').val('')
            $('#StartingDate').val('')
        })
